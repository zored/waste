...
export class PDFParser {
    ...
    _parseTransactions(lines) {
        // Собираем блоки: каждый блок начинается со строки, содержащей дату
        const blocks = [];
        let current = null;
        ...
        for (const line of lines) {
            ...
            if (current) current.tokens.push(...line.tokens);
        }
        if (current) blocks.push(current);
        ...
        for (const block of blocks) {
            const tx = this._parseBlock(block.tokens);
    ...
    _parseBlock(tokens) {
        ...
        // Ищем сумму: последний токен, похожий на число с запятой (справа налево)
        let amount = null;
        for (let i = tokens.length - 1; i >= 0; i--) {
            ...
            if (AMOUNT_RE.test(nNoRub)) {
                amount = this._parseAmount(nNoRub);
                break;
            }
            // ₽ как отдельный токен перед суммой
            if (CURRENCY_RE.test(n) && i > 0) {
                const prev = tokens[i - 1].replace(/\u00a0/g, ' ').trim();
                if (AMOUNT_RE.test(prev)) {
                    amount = this._parseAmount(prev);
                    break;
                }
            }
        }
        ...
        // Метатокены: всё кроме даты, времени, сумм, символов валют
        ...
        // В выписках Сбера последний токен — обычно категория, остальное — описание
        if (metaTokens.length >= 2) {
            category = metaTokens[metaTokens.length - 1];
            description = metaTokens.slice(0, -1).join(' ');
        }
